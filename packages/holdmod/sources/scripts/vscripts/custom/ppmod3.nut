  ...
  movesim = function(move, ftime = null, accel = 10, fric = 0, ground = Vector(0, 0, -1), grav = Vector(0, 0, -600), eyes = null) {
    ...
    if(relay) {
      ...
      ppmod.give("trigger_gravity", function(gravtrig, vel = finalvel, time = ftime + FrameTime()) {
        ...
        ppmod.trigger(GetPlayer().GetCenter() + Vector(256), Vector(128, 128, 128), gravtrig);
...
ppmod.ray <- function(start, end, ent = null, world = true, ray = null) {
  ...
  local bmin, bmax;
  if(ent.GetClassname() != "projected_wall_entity" && ent.GetClassname() != "player") {
    bmin = [pos.x, pos.y, pos.z];
    bmax = [pos.x, pos.y, pos.z];
    local a, b;
    ...
    bmin = [pos.x + mins[0], pos.y + mins[1], pos.z + mins[2]];
    bmax = [pos.x + maxs[0], pos.y + maxs[1], pos.z + maxs[2]];
  }
...
ppmod.screen <- function(func) {
  ppmod.give("vgui_screen", function(ent, func = func) {
    ppmod.keyval(ent, "Effects", 1);
    ppmod.keyval(ent, "RenderMode", 1);
    ppmod.keyval(ent, "RenderAmt", 254);
    ppmod.keyval(ent, "PanelName", "level_placard_screen");
    func({

      GetEntity = function(e = ent) { return e },
      SetOrigin = function(pos, e = ent) { e.SetOrigin(pos) },
      SetAngles = function(x,y,z, e = ent) { e.SetAngles(x,y,z) },
      SetSize = function(w,h, e = ent) {
        ppmod.keyval(e, "Width", w);
        ppmod.keyval(e, "Height", h);
      }
      SetTexture = function(tex, e = ent) {
        ppmod.keyval(e, "OverlayMaterial", tex);
      }
