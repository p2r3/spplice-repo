...
::stringify <- function (table, level = 1) {
  ...
  foreach (key, val in table) {
    ...
    switch (typeof val) {
      ...
      case "table": { output += ::stringify(table, level + 1); break; }
      case "array": { output += pparray(val); break; }
      case "string": { output += "\"" + val + "\""; break; }
      case "null": { output += "null"; break; }
      default: output += val;
